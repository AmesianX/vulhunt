...
use bias::loader::meta::{
    BA2ComponentAttributes, EFIModuleAttributes, EFIStandaloneAttributes, PosixELFAttributes,
    WindowsPEAttributes,
};
...
use bias::platform::efi::{
    EFIModule, EFIModuleAttribute, EFIModuleBuilder, EFIStandalone, EFIStandaloneAttribute,
    EFIStandaloneBuilder,
};
...
pub(crate) fn build_lifter_for_platform(
    ...
    let convention = match platform.name() {
        EFIStandalone::NAME | EFIModule::NAME => "efi",
...
pub(crate) fn infer_platform(
    ...
    let guid = attrs.and_then(|a| a.get_attr::<Uuid>("guid"));

    match platform_name.as_deref() {
        Some(n) if n.starts_with("efi-") && guid.is_some() => {
            let kind = infer_efi_module_kind(bytes, attrs);
            let guid = guid.unwrap();
            let is_te = matches!(kind, EFIModuleType::PeiModule);

            let mut builder = EFIModuleBuilder::new();
            builder.push(EFIModuleAttribute::name(file_name.clone()));
            builder.push(EFIModuleAttribute::kind(kind));
            builder.push(EFIModuleAttribute::guid(guid));

            if let Some(ref arch) = architecture {
                builder.push(EFIModuleAttribute::architecture(arch.clone()));
            }
            builder.push(EFIModuleAttribute::loader_metadata(loader_meta.clone()));

            let component_attrs = EFIModuleAttributes::new(guid, kind)
                .with_architecture(architecture)
                .with_loader_metadata(loader_meta);

            let attrs = if is_te {
                BA2ComponentAttributes::EFIModuleTE(component_attrs)
            } else {
                BA2ComponentAttributes::EFIModulePE(component_attrs)
            };
            Ok((
                builder
