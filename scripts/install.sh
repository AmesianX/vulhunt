...
BIN_DIR="${VULHUNT_BIN_DIR:-$INSTALL_DIR}"
...
install() {
    ...
    if [ -d "$temp_dir/extracted/lib" ]; then
        mkdir -p "$BIN_DIR/lib"
        mv "$temp_dir/extracted/lib/"* "$BIN_DIR/lib/"
        find "$BIN_DIR/lib" -type f -exec chmod +x {} \;
    fi
    ...
    info "Building type libraries..."
    find "$data_dir/platforms/posix/types" "$data_dir/platforms/uefi/types" -name "*.bin" 2>/dev/null | while read -r bin_file; do
        h_file="${bin_file%.bin}.h"
        if [ -f "$h_file" ]; then
            PATH="$BIN_DIR:$PATH" "$BIN_DIR/bias-tutil" build "$h_file"
        fi
    done

    info "Building processor specifications..."
    PATH="$BIN_DIR:$PATH" "$BIN_DIR/bias-lutil" --data "$data_dir/processors"
